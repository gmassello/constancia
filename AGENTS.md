...
- **No secrets in git.** `.env` is ignored; `.env.example` lists every key anyone has to set, with an empty value. The four tuning knobs that are defaulted in `app/config.py` are deliberately left out of it and documented in `docs/OPERATIONS.md` instead. Never print a secret value, not even truncated.
