...
- **Everything in the repo is written in English**: code, identifiers, docs, commits, PRs. Two exceptions, both of them content rather than code: what the patient hears (prompt fragments, questions, escalation and goodbye lines in `app/packs.py`), and the Spanish half of the bilingual interface (`web/src/landing/copy.ts`, `web/src/panel/copy.ts`), where English is the annotated base and Spanish the translation. The agent speaks Rioplatense Spanish.
