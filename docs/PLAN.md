...
- STT in English: `speech_model=universal-streaming-english`, `language_codes=["en"]`, `encoding=pcm_mulaw`, `sample_rate=8000`. Audio goes from Twilio to AssemblyAI with a single `base64.b64decode`; no resampling. Raw `websockets` instead of the `assemblyai` SDK (simple JSON protocol, full asyncio control). Post-call: pre-recorded API with `entity_detection` + `sentiment_analysis` (`punctuate=true`).
