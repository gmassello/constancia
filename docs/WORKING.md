...
```bash
uv run pytest -q          # 172 tests, the 3 that need a database skipped
uv run ruff check .
...
Fourteen test files, 172 collected with no environment variable set. Every one of them passes
