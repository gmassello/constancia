...
```bash
uv run pytest -q          # 171 tests, the 3 that need a database skipped
uv run ruff check .
...
Fourteen test files, 171 collected with no environment variable set. Every one of them passes
