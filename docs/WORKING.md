...
```bash
uv run pytest -q          # 169 tests, the 3 that need a database skipped
uv run ruff check .
...
Fourteen test files, 169 collected with no environment variable set. Every one of them passes
