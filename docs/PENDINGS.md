...
| 2 | **The three rehearsals and the take.** Closes **C2** and **C3** | you and me | [`video-script.md`](video-script.md) § *The day, in order*. The order is not negotiable: the three calls back to back come **before** the final reset, never after |
| 3 | **Assemble `demo.mp4`, and the pipeline that would do it.** ~~`fit-to-audio.py --beats`, then `build-video.sh`.~~ **Neither script exists**: not in the repo, not on disk, and `git log --all` finds no commit that ever added or removed them — they were documented, never written. What exists is their output, in the git-ignored `video/out/`: `narration.wav` (measured 267.781167 s, the 267.8 s five documents publish), 53 clips, the silences, `concat.txt`, `captions.srt`, `timing.txt` and `endcard.png` — 49 MB that cannot be regenerated. `demo.mp4`, `raw-fitted.mov` and `demo.en.srt` do not exist, and the 20 Sep take is split across two `.mov` files | me, after the take | **Decide the route before the take, not after**: write the assembly step, or plan to cut it by hand with ffmpeg. Either way `video/out/` gets backed up off the repo first — it is the one irreplaceable artifact in this project |
| 4 | **Share the deck and submit on lablab** | you | The artifact is linked from [`SUBMISSION.md`](SUBMISSION.md); it has to be shared before it can be uploaded |
...
that leaves for row 2: the raw screen recording has to run between **3:19 and 5:45**, or
`build-video.sh` refuses it.

**Checkpoints.** C1 is closed. **C2** (three real calls back to back, no reset, the third superseding
the 7/10) and **C3** (the panel live during a call) close during the recording session — row 2.
...
| 7 | Key phrases | **Built and unmeasured.** `auto_highlights` is on in the pre-recorded request, the ten highest-ranked phrases land in `calls.analysis`, the panel shows them under the entities and the sentiment, and `with_phrases` adds them to the next call's `keyterms_prompt` behind the terms that came from facts. What is open is the number: `make smoke-keyphrases URL=<recording url>` transcribes the same recording twice, with and without the feature, and writes `scripts/keyphrases-measured.json` with every phrase, its rank, its count and how many of them the fact vocabulary did not already hold. It needs a real Twilio recording, which the `make smoke-analysis` step of rehearsal 1 produces. **That run is a gate, not a formality**: AssemblyAI's own documentation warns that many terms, or common ones, can cause *"overcorrections and hallucinations"*, so if the phrases come back as common English or add nothing over the fact vocabulary, the half that feeds the recogniser comes out and the half that shows them in the panel stays. |
