...
| 2 | Commitments | **Closed for what a vertical can enumerate, and the rest is now measured rather than counted.** The action vocabulary lives in `pack.actions`, so a postpartum or chronic promise is scored deterministically by its own pack with no key and no network. What was open here was recall, and it was open partly because nothing computed it: the sample's labels were a comment in prose. They are data now, `make smoke-jev` prints recall, precision and the margin, and it writes [`../scripts/jev-measured.json`](../scripts/jev-measured.json) with every probability and a hash of the question wording. **Measured 25 Sep, wording `117b8a3c18ac`: recall 0.50, precision 1.00, margin 0.78.** The rewritten question — an ordinary action counts when it is done for the recovery, and following the professional's advice counts too — did not move recall, but it moved *"I will put my feet up whenever I sit down"* from 0.15 to 0.50 and the two it already caught to 0.96 and 0.97, with the best everyday plan down to 0.02. What is still open: the floor at 0.8 excludes that 0.50, and **one of the two misses may be mislabelled** — *"I will take the pram to the corner every afternoon"* carries nothing in the sentence that says it is about her body. Deciding that is the next step, not another wording. Also open: Zero Data Retention is a Pro tier and this is a hobby plan, so `JEV_ZERO_RETENTION` defaults off and the sentence falls under the gateway's ordinary retention; asking for it anyway is a 403 on every call, which is how we found out. The gateway also rate-limits a run of eight in a row with a 429, which the client turns into the same `None`. |
